...
# simple-dummy macOS Build Script
# This script builds the simple-dummy application for macOS
...
create_static_package() {
    if [ "$ENABLE_STATIC_LINKING" = "ON" ]; then
        ...
        STATIC_DIR="$DIST_DIR/simple-dummy-$VERSION-static-macos"
        mkdir -p "$STATIC_DIR"
        
        # Copy binary and files
        cp "$BUILD_DIR/simple-dummy" "$STATIC_DIR/"
        ...
        # Create tarball
        cd "$DIST_DIR"
        tar -czf "simple-dummy-$VERSION-static-macos.tar.gz" "simple-dummy-$VERSION-static-macos"
        rm -rf "simple-dummy-$VERSION-static-macos"
        
        print_success "Static binary package created: simple-dummy-$VERSION-static-macos.tar.gz"
    fi
}
