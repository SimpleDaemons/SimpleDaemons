#!/bin/bash
# Generic Linux build script for simple-dummy
...
build_project() {
    print_status "Building simple-dummy..."
...
install_project() {
    if [ "$INSTALL" = "true" ]; then
        print_status "Installing simple-dummy..."
        cd "$BUILD_DIR"
        
        sudo make install
        
        # Test installation
        if command_exists simple-dummy; then
...
create_service() {
    if [ "$SERVICE" = "true" ]; then
        ...
        sudo tee /etc/systemd/system/simple-dummy.service > /dev/null << EOF
[Unit]
Description=A test application for demonstrating the standardized build system and static binary creation capabilities
...
User=simple-dummy
Group=simple-dummy
ExecStart=/usr/local/bin/simple-dummy --daemon
Restart=always
RestartSec=5
...
EOF
        ...
        sudo useradd --system --no-create-home --shell /bin/false simple-dummy 2>/dev/null || true
        ...
        print_status "Use 'sudo systemctl start simple-dummy' to start the service"
    fi
}
...
show_usage() {
    cat << EOF
simple-dummy - Linux Build Script
...
# Main execution
main() {
    print_status "Starting simple-dummy build process..."
    ...
    print_success "simple-dummy build process completed successfully!"
}
